...
implements Iterable<java.util.jar.JarEntry> {
    ...
    private JarFile(RandomAccessDataFile rootFile, String pathFromRoot, RandomAccessData data, JarEntryFilter filter2, AbstractJarFile.JarFileType type, Supplier<Manifest> manifestSupplier) throws IOException {
        ...
        this.entries = parser.addVisitor(new JarFileEntries(this, filter2));
        this.type = type;
        parser.addVisitor(this.centralDirectoryVisitor());
        try {
            this.data = parser.parse(data, filter2 == null);
        }
    ...
    private JarFile createJarFileFromDirectoryEntry(JarEntry entry) throws IOException {
        ...
        JarEntryFilter filter2 = candidate -> {
            if (candidate.startsWith(name) && !candidate.equals(name)) {
                return candidate.substring(name.length());
            }
            return null;
        };
        return new JarFile(this.rootFile, this.pathFromRoot + "!/" + entry.getName().substring(0, name.length() - 1), this.data, filter2, AbstractJarFile.JarFileType.NESTED_DIRECTORY, this.manifestSupplier);
    }
    ...
    implements Enumeration<java.util.jar.JarEntry> {
        ...
        JarEntryEnumeration(Iterator<JarEntry> iterator2) {
