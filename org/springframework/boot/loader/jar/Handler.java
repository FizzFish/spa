...
extends URLStreamHandler {
    ...
    private JarFile getRootJarFile(String name) throws IOException {
        try {
            ...
            Map<File, JarFile> cache2 = rootFileCache.get();
            JarFile jarFile = result = cache2 != null ? cache2.get(file) : null;
            if (result == null) {
    ...
    static void addToRootFileCache(File sourceFile, JarFile jarFile) {
        Map<File, JarFile> cache2 = rootFileCache.get();
        if (cache2 == null) {
            cache2 = new ConcurrentHashMap<File, JarFile>();
            rootFileCache = new SoftReference<Map<File, JarFile>>(cache2);
        }
        cache2.put(sourceFile, jarFile);
    }
