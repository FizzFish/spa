...
implements FileHeader {
    ...
    void load(byte[] data, int dataOffset, RandomAccessData variableData, long variableOffset, JarEntryFilter filter2) throws IOException {
        this.header = data;
        ...
        if (filter2 != null) {
            this.name = filter2.apply(this.name);
        }
        this.extra = NO_EXTRA;
    ...
    static CentralDirectoryFileHeader fromRandomAccessData(RandomAccessData data, long offset, JarEntryFilter filter2) throws IOException {
        CentralDirectoryFileHeader fileHeader = new CentralDirectoryFileHeader();
        byte[] bytes = data.read(offset, 46L);
        fileHeader.load(bytes, 0, data, offset, filter2);
        return fileHeader;
    }
