...
extends Launcher {
    ...
    private void loadResource(InputStream resource) throws Exception {
        ...
        for (Object key2 : Collections.list(this.properties.propertyNames())) {
            String text2 = this.properties.getProperty((String)key2);
            String value = SystemPropertyUtils.resolvePlaceholders(this.properties, text2);
            if (value == null) continue;
            this.properties.put(key2, value);
        }
        if ("true".equals(this.getProperty(SET_SYSTEM_PROPERTIES))) {
            this.debug("Adding resolved properties to System properties");
            for (Object key2 : Collections.list(this.properties.propertyNames())) {
                String value = this.properties.getProperty((String)key2);
                System.setProperty((String)key2, value);
            }
        }
    }
    ...
    protected ClassLoader createClassLoader(Iterator<Archive> archives) throws Exception {
        ...
        ClassLoader loader2 = new LaunchedURLClassLoader(urls.toArray(NO_URLS), this.getClass().getClassLoader());
        this.debug("Classpath for custom loader: " + urls);
        loader2 = this.wrapWithCustomClassLoader(loader2, customLoaderClassName);
    ...
    public static String toCamelCase(CharSequence string) {
        ...
        int pos = 0;
        while (matcher.find()) {
            builder2.append(PropertiesLauncher.capitalize(string.subSequence(pos, matcher.end()).toString()));
            pos = matcher.end();
        }
        builder2.append(PropertiesLauncher.capitalize(string.subSequence(pos, string.length()).toString()));
    ...
    implements Iterable<Archive> {
        ...
        private List<Archive> getNestedArchives(String path) throws Exception {
            ...
            PrefixMatchingArchiveFilter filter2 = new PrefixMatchingArchiveFilter(root);
            List<Archive> archives = this.asList(parent.getNestedArchives(null, filter2));
        ...
        private List<Archive> asList(Iterator<Archive> iterator2) {
