...
public class SparseInfoFlowSolver extends InfoflowSolver {
    ...
    private boolean isForward() {
        return !(icfg instanceof BackwardsInfoflowCFG);
    }
    ...
    protected void propagate(
        ...
        @NotNull Unit relatedCallSite,
        boolean isUnbalancedReturn
    ) {
        if (targetVal.getAccessPath().getPlainValue() == null 
            || targetVal.getAccessPath().equals(((Abstraction) zeroValue).getAccessPath())) {
            super.propagate(sourceVal, target, targetVal, relatedCallSite, isUnbalancedReturn);
            return;
        }

        SootMethod method = icfg.getMethodOf(target);
        DirectedGraph graph = icfg.getOrCreateUnitGraph(method);
        UnitGraph unitGraph = (UnitGraph) graph;
        AP ap = AP.Companion.get(targetVal);

        Set<Unit> uses = Set.copyOf(isBackward() 
        ...
        for (Pair<Unit, Abstraction> pair : propagates) {
            ...
            if (isBackward()) {
                ...
                if (dominator.getUnit() != null && dominator.getUnit() instanceof IfStmt) {
                    ...
                    super.propagate(sourceVal, useUnit, newAbs, relatedCallSite, isUnbalancedReturn);
                }
            }
            super.propagate(sourceVal, useUnit, toVal, relatedCallSite, isUnbalancedReturn);
        }
    }
